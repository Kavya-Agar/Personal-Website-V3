...
const commands: Command[] = [
  ...
  },
  {
    ...
    action: () => { window.open("https://github.com/kavyaagar", "_blank"); },
  },
  {
    label: "LinkedIn",
    category: "Links",
...
export function CommandPalette({ open, onClose }: Props) {
  ...
  // Flat index map so selected index lines up with grouped display
  let flatIndex = 0;
    ...
    >
      <div
        ...
        style={{
          ...
          boxShadow: "0 24px 60px rgba(0,0,0,0.6), 0 0 0 1px rgba(0,200,83,0.08)",
      ...
      >
        {/* Search bar */}
        <div className="flex items-center gap-3 px-4 py-3" style={{ borderBottom: "1px solid #21262d" }}>
          ...
          />
          <kbd className="font-mono text-[10px] px-1.5 py-0.5 rounded" style={{ color: "#484f58", background: "#0d1117", border: "1px solid #30363d" }}>
            ESC
          </kbd>
        </div>
        ...
        <div className="overflow-y-auto" style={{ maxHeight: 360 }}>
          {filtered.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-10 gap-2">
              <span className="font-mono text-[11px]" style={{ color: "#484f58" }}>// no results</span>
            ...
            <div className="py-2">
              {grouped.map(({ cat, items }) => (
                <div key={cat}>
                  <p className="px-4 pt-3 pb-1 font-mono text-[9px] uppercase tracking-widest" style={{ color: "#484f58" }}>
                    {cat}
                  </p>
                  {items.map((cmd) => {
                    const idx = flatIndex++;
                    ...
                    return (
                      <button
                        ...
                        style={{
                          background: isSelected ? "rgba(0,200,83,0.06)" : "transparent",
                      ...
                      >
                        <span style={{ color: isSelected ? "#00c853" : "#484f58", flexShrink: 0 }}>
                          {cmd.icon}
                        </span>
        ...
        {/* Footer */}
        <div className="px-4 py-2.5 flex items-center gap-4 font-mono text-[10px]" style={{ borderTop: "1px solid #21262d", color: "#484f58" }}>
          <span className="flex items-center gap-1">
            <kbd className="px-1 py-0.5 rounded" style={{ background: "#0d1117", border: "1px solid #30363d" }}>↑</kbd>
            <kbd className="px-1 py-0.5 rounded" style={{ background: "#0d1117", border: "1px solid #30363d" }}>↓</kbd>
            navigate
          </span>
          <span className="flex items-center gap-1">
          ...
          <span className="ml-auto" style={{ color: "#00c853" }}>⌘K</span>
        </div>
      </div>
    </div>
  );
